...
proc createnewcontent
	...
	repeat
		listbox_getfile [cont_header] = 'Select template file', [template_dir] # '\*.html', '1'
		if [errorcode] = '-2'
	...
	repeat
		...
		if [block] ! ''
			...
			if [type] = ''
				...
				upvar [block] = [block]
				[cont_header] # [type] # [block] # [udms_sep]
			endif
		endif
	until [block] = ''
...
proc getContentHeader: [filename], [*header], [*template]
	...
	gettok [template] = [head], [udms_sep], '1'
	[i] = '1'
	repeat
		[i] + '1'
		gettok [tmp] = [head], [udms_sep], [i]
		if [tmp] ! ''
...
proc buildChapterList: [filename], [*info]
	...
	repeat
		...
		if [item] ! '' & [item] ! 'EOF'
			gettok [info:[i],'1'] = [item], [udms_sep], '1'
			[j] = '1'
			repeat
				[j] + '1'
				gettok [tmp] = [item], [udms_sep], [j]
				if [tmp] ! ''
					[info:[i],[j]] = [tmp]
				endif
			until [tmp] = ''
...
proc writeContentFile: [filename], [*info], [*blocks], [tfilename]
	...
	if [blocks,'0'] > '0'
		...
		repeat
			[i] + '1'
			[cont_header] # [udms_sep] # [blocks,[i],'2'] # [blocks,[i],'1']
		until [i] = [blocks,'0']
	endif
...
proc generateHTMLContent: [filename], [*info], [*blocks], [use_template]
	...
	repeat
		...
		if [info[i],'1'] ! ''
			...
			replacevar [outfile] = [outfile], '<br>', '<br>' # [new_line]
...
proc chapterEditor: [title], [*chapter]
	...
	repeat
		...
		if [text_blocks:[j]-'1','2'] = '0'
			...
			newdialog 'ed:edit' # [j], 'edit', '10|' # [y] # '|800|150'
			[y] + '140'
		endif
...
rem Main routines -----------------------------------------------------------------------------------------------------------
%getcont
findfiles [content_files] = [content_dir] # '\*.cont', '1'
...
repeat
	if [menu_mode] = '0'
	traymenu [cmd] = [contentfile] # ' :: UDMS', 'Create chapter|Edit chapter|Delete chapter|-|Save content file|Manually edit content|-|Generate HTML|Generate HTML (Batch Mode)|-|Create new content file|Change content file|-|About...|Switch menu|Quit'
	else
	listbox [cmd] = [contentfile] # ' :: UDMS', 'Create chapter|Edit chapter|Delete chapter||Save content file|Manually edit content||Generate HTML|Generate HTML (Batch Mode)||Create new content file|Change content file||About...|Switch menu|Quit'
	endif
until [cmd] ! ''
...
elseif [cmd] = 'Quit'
	rem writeContentFile [contentfile], [content_info], [text_blocks], [template_file]
	halt
endif
